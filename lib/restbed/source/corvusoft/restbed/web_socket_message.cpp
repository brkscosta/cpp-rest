...
WebSocketMessage::WebSocketMessage(const WebSocketMessage::OpCode code, const Bytes& data)
    : WebSocketMessage(code, data, 0)
{
    return;
}

WebSocketMessage::WebSocketMessage(const WebSocketMessage::OpCode code, const string& data)
    : WebSocketMessage(code, data, 0)
{
    return;
}

WebSocketMessage::WebSocketMessage(const WebSocketMessage::OpCode code, const Bytes& data, const uint32_t mask)
    : m_pimpl(new WebSocketMessageImpl)
{
    m_pimpl->m_data = data;
    m_pimpl->m_mask = mask;
...
WebSocketMessage::WebSocketMessage(const WebSocketMessage::OpCode code, const string& data, const uint32_t mask)
    : WebSocketMessage(code, String::to_bytes(data), mask)
{
    return;
}
