...
void Service::start(const shared_ptr<const Settings>& settings)
{
    ...
    stable_sort(m_pimpl->m_rules.begin(), m_pimpl->m_rules.end(),
        [](const shared_ptr<const Rule>& lhs, const shared_ptr<const Rule>& rhs)
    ...
    else
    {
        ...
        {
            signals.push_back(async(launch::async, [this]() { m_pimpl->m_io_service->run(); }));
        }
                ...
                signals.erase(std::remove_if(signals.begin(), signals.end(),
                                  [](future<void>& signal)
                                  {
                                      return (signal.wait_for(milliseconds(5)) == future_status::ready)
                                                 ? (signal.get(), true)
                                                 :  // this will throw if the worker
                                                    // thread ended with an exception
                                                 false;
                                  }),
                    signals.end());
            }
...
void Service::publish(const shared_ptr<const Resource>& resource)
{
    ...
    {
        throw invalid_argument("Resource would pollute namespace. Please ensure all published "
                               "resources have unique paths.");
    }
...
void Service::suppress(const shared_ptr<const Resource>& resource)
{
    ...
    for (const auto& path : resource->m_pimpl->m_paths)
    {
        ...
        {
            m_pimpl->log(
                Logger::WARNING, String::format("Failed to suppress resource route '%s'; Not Found!", path.data()));
        }
    }
}
...
void Service::schedule(const function<void(void)>& task, const milliseconds& interval)
{
    ...
    timer->async_wait(
        [this, task, interval, timer](const error_code&)
        {
            task();
...
void Service::set_authentication_handler(
    const function<void(const shared_ptr<Session>, const function<void(const shared_ptr<Session>)>&)>& value)
{
    if (is_up())
    {
        throw runtime_error("Runtime modifications of the service are prohibited.");
