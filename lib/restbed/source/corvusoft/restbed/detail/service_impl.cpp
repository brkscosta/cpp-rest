...
ServiceImpl::ServiceImpl(void)
    : m_uptime(steady_clock::time_point::min()), m_logger(nullptr), m_supported_methods(), m_settings(nullptr),
      m_io_service(make_shared<::io_service>()), m_signal_set(nullptr), m_session_manager(nullptr),
      m_web_socket_manager(nullptr), m_rules(), m_workers_stopped(),
#ifdef BUILD_SSL
      m_ssl_settings(nullptr), m_ssl_context(nullptr), m_ssl_acceptor(nullptr),
#endif
#ifdef BUILD_IPC
      m_ipc_acceptor(nullptr),
#endif
      m_acceptor(nullptr), m_resource_paths(), m_resource_routes(), m_ready_handler(nullptr), m_signal_handlers(),
      m_not_found_handler(nullptr), m_method_not_allowed_handler(nullptr), m_method_not_implemented_handler(nullptr),
      m_failed_filter_validation_handler(nullptr), m_error_handler(ServiceImpl::default_error_handler),
...
void ServiceImpl::signal_handler(const error_code& error, const int signal_number) const
{
    ...
    {
        log(Logger::WARNING,
            String::format("Failed to process signal '%i', '%s'.", signal_number, error.message().data()));
        return;
    }
...
void ServiceImpl::https_start(void)
{
    if (m_ssl_settings not_eq nullptr)
    {
        ...
        m_ssl_context->set_password_callback(
            [passphrase](size_t, asio::ssl::context::password_purpose) { return passphrase; });
        ...
        options = (m_ssl_settings->has_enabled_default_workarounds())
                      ? options | asio::ssl::context::default_workarounds
                      : options;
        options = (m_ssl_settings->has_enabled_single_diffie_hellman_use())
        ...
        if (not m_ssl_settings->get_bind_address().empty())
        {
            ...
            m_ssl_acceptor =
                make_shared<tcp::acceptor>(*m_io_service, tcp::endpoint(address, m_ssl_settings->get_port()));
        }
        else
        {
            m_ssl_acceptor =
                make_shared<tcp::acceptor>(*m_io_service, tcp::endpoint(tcp::v6(), m_ssl_settings->get_port()));
        }
...
void ServiceImpl::create_ssl_session(
    const shared_ptr<asio::ssl::stream<tcp::socket>>& socket, const error_code& error) const
{
    if (not error)
    {
            ...
            [this, socket](const error_code& error)
            {
                ...
                if (m_settings->get_keep_alive())
                {
                    connection->set_keep_alive(m_settings->get_keep_alive_start(),
                        m_settings->get_keep_alive_interval(), m_settings->get_keep_alive_cnt());
                }

                m_session_manager->create(
                    [this, connection](const shared_ptr<Session> session)
                    {
                        ...
                        session->m_pimpl->m_request->m_pimpl->m_socket->start_read(
                            session->m_pimpl->m_request->m_pimpl->m_buffer, "\r\n\r\n",
...
void ServiceImpl::create_ipc_session(const shared_ptr<stream_protocol::socket>& socket, const error_code& error) const
{
    if (not error)
    {
        ...
        if (m_settings->get_keep_alive())
        {
            connection->set_keep_alive(m_settings->get_keep_alive_start(), m_settings->get_keep_alive_interval(),
...
string ServiceImpl::sanitise_path(const string& path) const
{
    ...
    for (auto folder : String::split(path, '/'))
    {
        if (folder.front() == '{' and folder.back() == '}')
        {
            ...
            {
                throw runtime_error(
                    String::format("Resource path parameter declaration is malformed '%s'.", folder.data()));
            }
...
{
    log(Logger::INFO, String::format("'%s' resource route not found '%s'.", session->get_origin().data(),
...
{
    log(Logger::INFO, String::format("'%s' '%s' method not allowed '%s'.", session->get_origin().data(),
                          session->get_request()->get_method().data(), session->get_request()->get_path().data()));
...
{
    log(Logger::INFO, String::format("'%s' '%s' method not implemented '%s'.", session->get_origin().data(),
                          session->get_request()->get_method().data(), session->get_request()->get_path().data()));
...
{
    log(Logger::INFO, String::format("'%s' failed filter validation '%s'.", session->get_origin().data(),
                          session->get_request()->get_path().data()));
...
{
    log(Logger::INFO,
        String::format("Incoming '%s' request from '%s' for route '%s'.", session->get_request()->get_method().data(),
            session->get_origin().data(), session->get_request()->get_path().data()));
    ...
    rule_engine(session, m_rules,
        ...
        {
            const auto resource_route = find_if(m_resource_routes.begin(), m_resource_routes.end(),
                bind(&ServiceImpl::resource_router, this, session, _1));
            ...
            {
                rule_engine(session, session->m_pimpl->m_resource->m_pimpl->m_rules,
                    [this](const shared_ptr<Session> session)
                    {
                        if (session->is_closed())
                        {
...
function<void(const shared_ptr<Session>)> ServiceImpl::find_method_handler(const shared_ptr<Session> session) const
{
    ...
    for (auto handler = method_handlers.first; handler not_eq method_handlers.second and method_handler == nullptr;
         handler++)
    {
...
bool ServiceImpl::resource_router(
    const shared_ptr<Session> session, const pair<string, shared_ptr<const Resource>>& route) const
{
    const auto request = session->get_request();
...
{
    smatch matches;
    static const regex pattern("^([0-9a-zA-Z]*) ([a-zA-Z0-9:@_~!,;=#%&'\\-\\.\\/\\?\\$\\(\\)\\*\\+]+) "
                               "(HTTP\\/[0-9]\\.[0-9])\\s*$");
    string data = "";
    getline(stream, data);
    ...
    {
        throw runtime_error("Your client has issued a malformed or illegal "
                            "request status line. That’s all we know.");
    }

    const string protocol = matches[3].str();
    const auto delimiter = protocol.find_first_of("/");

    return map<string, string>{{"path", matches[2].str()}, {"method", matches[1].str()},
        {"version", protocol.substr(delimiter + 1)}, {"protocol", protocol.substr(0, delimiter)}};
}
...
const multimap<string, string> ServiceImpl::parse_request_headers(istream& stream)
{
    ...
    while (getline(stream, data) and data not_eq "\r")
    {
        ...
        {
            throw runtime_error("Your client has issued a malformed or illegal "
                                "request header. That’s all we know.");
        }
...
const function<void(const int, const exception&, const shared_ptr<Session>)> ServiceImpl::get_error_handler(
    const shared_ptr<Session>& session) const
{
    return (session->m_pimpl->m_resource not_eq nullptr and
               session->m_pimpl->m_resource->m_pimpl->m_error_handler not_eq nullptr)
               ? session->m_pimpl->m_resource->m_pimpl->m_error_handler
               : m_error_handler;
}
}  // namespace detail
}  // namespace restbed
