...
namespace detail
{
WebSocketManagerImpl::WebSocketManagerImpl(void) : m_logger(nullptr), m_sockets()
{
    return;
}
...
shared_ptr<WebSocketMessage> WebSocketManagerImpl::parse(const Bytes& packet)
{
    ...
    message->set_reserved_flags(
        (byte & 0x40) ? true : false, (byte & 0x20) ? true : false, (byte & 0x10) ? true : false);
