...
namespace detail
{
IPCSocketImpl::IPCSocketImpl(asio::io_context& context, const shared_ptr<stream_protocol::socket>& socket,
    const std::string& path, const shared_ptr<Logger>& logger)
    : SocketImpl(context), m_error_handler(nullptr), m_is_open(socket->is_open()), m_path(path), m_pending_writes(),
      m_logger(logger), m_timeout(0), m_io_service(context), m_timer(make_shared<asio::steady_timer>(m_io_service)),
      m_strand(make_shared<io_service::strand>(m_io_service)), m_socket(socket)
{
    return;
}
...
{
    m_socket->async_connect(stream_protocol::endpoint(m_path),
...
void IPCSocketImpl::start_write(
...
void IPCSocketImpl::start_read(const shared_ptr<asio::streambuf>& data, const size_t length,
    const function<void(const error_code&, size_t)>& callback)
{
    m_strand->post([this, data, length, callback] { read(data, length, callback); });
}

void IPCSocketImpl::start_read(const shared_ptr<asio::streambuf>& data, const string& delimiter,
    const function<void(const error_code&, size_t)>& callback)
{
...
shared_ptr<IPCSocketImpl> IPCSocketImpl::shared_from_this(void)
{
    ...
    // return shared_ptr< IPCSocketImpl >( this ); //test for circular reference
    // and memory leak.
}
...
void IPCSocketImpl::write(void)
{
    if (m_is_open)
    {
        ...
        m_timer->async_wait(
            m_strand->wrap(bind(&IPCSocketImpl::connection_timeout_handler, this, shared_from_this(), _1)));
...
void IPCSocketImpl::read(const shared_ptr<asio::streambuf>& data, const size_t length,
    const function<void(const error_code&, size_t)>& callback)
{
    m_timer->cancel();
    m_timer->expires_from_now(m_timeout);
...
void IPCSocketImpl::read(const shared_ptr<asio::streambuf>& data, const string& delimiter,
    const function<void(const error_code&, size_t)>& callback)
{
    m_timer->cancel();
    m_timer->expires_from_now(m_timeout);
