...
{
    m_socket->start_read(2, bind(&WebSocketImpl::parse_flags, this, _1, socket),
        [this, socket](const error_code code)
        {
...
void WebSocketImpl::parse_flags(const Bytes data, const shared_ptr<WebSocket> socket)
{
    ...
    m_socket->start_read(length, bind(&WebSocketImpl::parse_length_and_mask, this, _1, data, socket),
        [this, socket](const error_code code)
        {
            if (m_error_handler not_eq nullptr)
            {
...
void WebSocketImpl::parse_length_and_mask(const Bytes data, Bytes packet, const shared_ptr<WebSocket> socket)
{
    ...
    m_socket->start_read(length, bind(&WebSocketImpl::parse_payload, this, _1, packet, socket),
        [this, socket](const error_code code)
        {
            if (m_error_handler not_eq nullptr)
            {
