...
SessionImpl::SessionImpl(void)
    : m_id(String::empty), m_request(nullptr), m_resource(nullptr), m_settings(nullptr), m_manager(nullptr),
      m_web_socket_manager(nullptr), m_headers(), m_context(), m_error_handler(nullptr), m_keep_alive_callback(nullptr),
...
void SessionImpl::fetch_body(const size_t length, const shared_ptr<Session> session,
    const function<void(const shared_ptr<Session>, const Bytes&)>& callback) const
{
...
const function<void(const int, const exception&, const shared_ptr<Session>)> SessionImpl::get_error_handler(void)
{
    ...
    auto error_handler = (m_resource not_eq nullptr and m_resource->m_pimpl->m_error_handler not_eq nullptr)
                             ? m_resource->m_pimpl->m_error_handler
                             : m_error_handler;
