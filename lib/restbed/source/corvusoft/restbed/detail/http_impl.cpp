...
Bytes HttpImpl::to_bytes(const shared_ptr<Request>& request)
{
    ...
    auto data =
        String::format("%s %s %s/%.1f\r\n", request->get_method().data(), path.data(), "HTTP", request->get_version());
...
void HttpImpl::socket_setup(const shared_ptr<Request>& request, const shared_ptr<const Settings>& settings)
{
    if (request->m_pimpl->m_socket == nullptr)
    {
        ...
        else if (String::uppercase(request->m_pimpl->m_protocol) == "LOCAL")
        {
            ...
            request->m_pimpl->m_socket =
                make_shared<IPCSocketImpl>(*request->m_pimpl->m_io_service, socket, settings->get_ipc_path());
        }
#endif
        else
        {
...
#endif
void HttpImpl::request_handler(const error_code& error, const shared_ptr<Request>& request,
...
void HttpImpl::write_handler(const error_code& error, const size_t, const shared_ptr<Request>& request,
    const function<void(const shared_ptr<Request>, const shared_ptr<Response>)>& callback)
{
    if (error)
    {
    ...
    request->m_pimpl->m_socket->start_read(
        request->m_pimpl->m_buffer, "\r\n", bind(read_status_handler, _1, _2, request, callback));
}
...
void HttpImpl::read_status_handler(const error_code& error, const size_t, const shared_ptr<Request>& request,
    const function<void(const shared_ptr<Request>, const shared_ptr<Response>)>& callback)
{
    if (error)
    {
...
void HttpImpl::read_headers_handler(const error_code& error, const size_t, const shared_ptr<Request>& request,
    const function<void(const shared_ptr<Request>, const shared_ptr<Response>)>& callback)
{
