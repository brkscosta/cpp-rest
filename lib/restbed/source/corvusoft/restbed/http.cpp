...
Bytes Http::to_bytes(const shared_ptr<Response>& value)
{
    ...
    auto data = String::format("%s/%.1f %i %s\r\n", value->get_protocol().data(), value->get_version(),
        value->get_status_code(), value->get_status_message().data());
...
future<shared_ptr<Response>> Http::async(const shared_ptr<Request> request,
    const function<void(const shared_ptr<Request>, const shared_ptr<Response>)>& callback,
    const shared_ptr<const Settings>& settings)
{
    ...
    {
        throw runtime_error("Not Implemented! Rebuild Restbed with SSL "
                            "funcationality enabled.");
    }
    ...
    {
        finished = false;
        completion_handler = [&finished](const shared_ptr<Request>, const shared_ptr<Response>) { finished = true; };
    }
    ...
    {
        request->m_pimpl->m_socket->connect(
            request->get_host(), request->get_port(), bind(HttpImpl::request_handler, _1, request, completion_handler));
    }
    else if (request->m_pimpl->m_socket not_eq nullptr)
    {
        request->m_pimpl->m_socket->start_write(
            Http::to_bytes(request), bind(HttpImpl::write_handler, _1, _2, request, completion_handler));
    }
    ...
    if (finished)
    {
        return std::async(
            std::launch::async,
            [](const shared_ptr<Request> request) -> shared_ptr<Response>
            {
