...
{
    return m_pimpl->m_request not_eq nullptr and m_pimpl->m_request->m_pimpl->m_socket not_eq nullptr and
...
void Session::close(const Bytes& body)
{
    ...
    m_pimpl->m_request->m_pimpl->m_socket->start_write(body,
        [this, session](const error_code& error, size_t)
        {
            if (error)
            {
            ...
            m_pimpl->m_manager->save(session,
                [this, session](const shared_ptr<Session>) { m_pimpl->m_request->m_pimpl->m_socket->close(); });
        });
}
...
void Session::close(const Response& response)
{
    ...
    m_pimpl->transmit(response,
        [this, session](const error_code& error, size_t)
        {
            ...
            m_pimpl->m_manager->save(
                session, [this](const shared_ptr<Session>) { m_pimpl->m_request->m_pimpl->m_socket->close(); });
        });
}
...
void Session::yield(const Bytes& body, const function<void(const shared_ptr<Session>)>& callback)
{
    ...
    m_pimpl->m_request->m_pimpl->m_socket->start_write(body,
        [this, session, callback](const error_code& error, size_t)
        {
            if (error)
            {
...
void Session::yield(const int status, const Bytes& body, const multimap<string, string>& headers,
    const function<void(const shared_ptr<Session>)>& callback)
{
    Response response;
    response.set_body(body);
...
void Session::fetch(const size_t length, const function<void(const shared_ptr<Session>, const Bytes&)>& callback)
{
    ...
    if (length > m_pimpl->m_request->m_pimpl->m_buffer->size())
    {
        ...
        m_pimpl->m_request->m_pimpl->m_socket->start_read(m_pimpl->m_request->m_pimpl->m_buffer, size,
            [this, session, length, callback](const error_code& error, size_t)
            {
                if (error)
                {
...
void Session::fetch(const string& delimiter, const function<void(const shared_ptr<Session>, const Bytes&)>& callback)
{
    ...
    m_pimpl->m_request->m_pimpl->m_socket->start_read(m_pimpl->m_request->m_pimpl->m_buffer, delimiter,
        [this, session, callback](const error_code& error, size_t length)
        {
            if (error)
            {
                const auto message = String::format("Fetch failed: %s", error.message().data());
...
void Session::upgrade(const int status, const multimap<string, string>& headers,
    const function<void(const shared_ptr<WebSocket>)>& callback)
{
    upgrade(status, empty_body, headers, callback);
}

void Session::upgrade(const int status, const Bytes& body, const multimap<string, string>& headers,
    const function<void(const shared_ptr<WebSocket>)>& callback)
{
...
void Session::upgrade(const int status, const string& body, const multimap<string, string>& headers,
    const function<void(const shared_ptr<WebSocket>)>& callback)
{
...
void Session::sleep_for(const milliseconds& delay, const function<void(const shared_ptr<Session>)>& callback)
{
    ...
    m_pimpl->m_request->m_pimpl->m_socket->sleep_for(delay,
        [session, callback, this](const error_code& error)
        {
            if (error)
            {
